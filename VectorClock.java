...
public class VectorClock implements ClockService<int[], TimeStampedMessage> {
	...
	private int id; // IDs can be assigned in MessagePasser after parsing the config file: alice 1, bob 2, charlie 3
	...
	public int compare(int[] message1, int[] message2) {
		 ...
		 if(less_than(message1, message2))
             return -1;
     else if(less_than(message2, message1))
             return 1;
     else
             return 0;
	}
	...
	public void clockSynchronize(TimeStampedMessage msg) {
           ...
           {
                   timestamp[i] = Math.max(timestamp[i], ((int[])msg.getTimeStamp())[i] );
           }
           timestamp[id]++;
